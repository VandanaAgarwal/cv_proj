...
model = VGG16(weights='imagenet', include_top=False, input_shape=(350,350,3))
model.summary()
input('enter a key...')
...
for epoch in range(epochs):
    ...
    model.save(os.path.join(SAVER, "model_350_350.h5"))  #saving the model on each epoch
    model.save_weights(os.path.join(SAVER, "model_weights_350_350.h5")) #saving the weights of model on each epoch
...
log_frame.to_csv("data/logs/Log_350_350.csv", index = False)
print(log_frame.head())
#import sys
#sys.exit(0)
'''

log = pd.read_csv("data/logs/Log_350_350.csv")
...
model_VGG = VGG16(weights='imagenet', include_top=False, input_shape=(350,350,3))
model_top = load_model("data/model_save/model_350_350.h5")
...
def make_prediction(path):
    ...
    for (x,y,w,h) in faces:
        ...
        face_clip = cv2.resize(face_clip, (350, 350))
...
FrameCapture('/home/vandana/cv_Project/temp/youtube/thappad.mp4')
